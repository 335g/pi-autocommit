...
## YOUR DESIGN CONSTRAINT (Agent 1 — MINIMISE)

MINIMIZE the interface — aim for 1–3 entry points max. Maximise leverage per entry point. You may fold the two callers' concerns behind ONE method if you can do it WITHOUT a mode flag that leaks, or keep two methods but make each maximally deep. Prefer fewer, deeper entry points. Reject any helper export that a caller could live without.

## Output (in this order)

1. Interface — TypeScript signatures (types, methods, params) plus invariants, ordering, error modes
2. Usage example — BOTH callers (single + reorganiser) using it
