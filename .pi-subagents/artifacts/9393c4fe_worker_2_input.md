...
## YOUR DESIGN CONSTRAINT (Agent 3 — OPTIMISE FOR COMMON CALLER)

OPTIMISE FOR THE MOST COMMON CALLER — make the default case trivial. The COMMON case is the checkpoint-commit single-message path at every turn_end (high-frequency, fire-and-forget). The reorganiser's group-splitting runs once per agent loop and is rarer. Design so the high-frequency caller is ONE LINE with zero ceremony, even if the reorganiser caller is slightly more verbose. The default LLM adapter must be lazy-loaded with zero caller involvement. Discriminate against cleverness that helps the rare case at the expense of the common one.

## Output (in this order)

1. Interface — TypeScript signatures (types, methods, params) plus invariants, ordering, error modes
2. Usage example — BOTH callers (single + reorganiser) using it, with the single path fitting on one line
