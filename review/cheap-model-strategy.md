...
## 実装優先順位（更新: 2026-06-06）

| 優先度 | 施策 | コスト | 効果 | リスク | 状態 |
|--------|------|--------|------|--------|------|
| 🔴 P0 | Few-shot 例をプロンプトに追加 | 低（プロンプト変更のみ） | 高（安価モデルで顕著） | 低（トークン増加のみ） | ✅ 実装済 |
| 🔴 P0 | JSON 正規表現リペア | 低（~80行追加） | 高（壊れたJSONの救出） | 低 | ✅ 実装済 |
| 🟡 P1 | 型アンカー（ファイル拡張子ヒント） | 低（~20行追加） | 中（型精度向上） | 低 | ⬜ 未着手 |
| 🟡 P1 | 分解再生成（品質ゲート付き） | 中（構造変更あり） | 中〜高 | 中（API コール増） | ⬜ 未着手 |
| 🟢 P2 | 型一貫性チェック | 低 | 低〜中 | 低 | ⬜ 延期（P0/P1効果測定後） |
| 🟢 P2 | ファイル-メッセージ関連性スコア | 中 | 低〜中 | 低 | ⬜ 延期 |
| 🔵 P3 | 多肢生成 + 投票 | 中 | 中 | 中（コスト2倍） | ⬜ 無期限延期 |
